...
## Introduction
The issue at hand is that with the provided rubric, senior tutor scores sometime differ greatly from the answer key, which is the proposed scoring of the sessions. This means that junior tutors may receivescores that greatly differ from the answer key, which makes it difficult to assess the junior tutors accurately. Given the set of 10 sessions with gradings from 14 tutors on a rubric with 8 criteria, I looked at the data to see if I could find any decipherable patterns that would be telling of the issues at hand. I will incorporate my code into my discussion to show how I arrived to these conclusions and how I was able to analyze the data. The preliminary conclusions I came to include a reworking/clarification of certain parts of the criteria, talking to certain tutors to check their understanding of the criteria, and rather than looking at the tutors specifically, but also considering using an averaging/dominance of data to determine the "average" of all tutors criteria.

## Analysis
When I looked at the criteria initially and tried judging the sessions, I found it to be difficult to definitively say whether the tutor accomplished the task at hand or not. It felt black and white, but generally these things are not too black and white since sometimes tutors may accomplish it mid-way or get started on the right track, but diverge from the criteria which would definitively earn them a 0 even though they may have did an OK but not great job. Based on this initial impression, I analyzed the data regarding the criterion first. 

### Criteria Misinterpretation

This led me to consider the misunderstandings of the criteriion in relation to the Senior Tutors and how they assessed the sessions. My end goal was to recognize possibly troubling parts of the criteria and confirming these results. To start, I aggregated the data for each session by all of the tutors and went on to locate the conflict within the decisions on each criteria. I did this with the following: 
...
#indicates that there are a couple controversial criteria:
...
```
I first made a function that finds "variance" which in this instance basically meant that it would look at a session (contains all the tutors scores (each criteria) for that session) and then it counts the number of Yes for that criteria and the number of No for that criteria and finds the difference and taking the absolute value of that. So adding it all up, we get the above list which I named total_variance. So we can see that, starting with the first criteria and increasing respectively, we have 104 more Yes than No...and so on. In this instance, whether Yes or No was more doesn't matter, it's to see the "conflict" in the decision. The smaller the number, the more conflict there was between tutors on either Yes/No for that criteria since it means the # of Yes was closer to the # of No. In that list total variance, the outlier here is the second to last one: 
...
Next, I looked to verify this result by doing a dominance comparison. I aggregated all the information on Yes/No for each session for each criteria and then whichever decision was more dominant, I used that as the "group" decision for that criteria for the session. I will call this group decision the "average" tutor. I then compared the "average" tutor to the answer key, since I wanted to confirm the controversial criteria 7, and also see how the dominance worked out in comparison to the answer key. So I did the following 
